...
async function match_level_words(question: any, content: any) {
  ...
  const intersection = new Set(
    [...questionSet].filter((word) => contentSet.has(word))
  );
...
const WriteEmailScoring = async (
  ...
  const isWordHighlighted = (word: string): boolean => {
    ...
    return Array.from(spanElements).some(
      (span) =>
        span.classList.contains("highlight") && span.textContent === word
    );
  };
  ...
  try {
    ...
    if (nlpscore > 1 || (wordCount <= 120 && wordCount >= 80)) {
      const checkTopic = answertopic ? content.includes(answertopic) : true;
      const checkAnswerQuestionContent =
        (await match_level_words(questionData.question_name, content)) || false;
      ...
      if (result && typeof result !== "string") {
        ...
        let rform = wordCount <= 120 && wordCount >= 80 ? 2 : 0;
        ...
        rcontent = Math.max(0, rcontent - total_wrong_words * 0.1);
    ...
    console.error(
      "Error in trying to store the WriteEmailScoring data: ",
      error
    );
  }
...
export default WriteEmailScoring;
